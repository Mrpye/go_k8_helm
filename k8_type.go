...
type K8 struct {
	...
	UseTokenConnection bool   `json:"use_token_connection" yaml:"use_token_connection"` //if true, use the token connection, otherwise use the kube config file
	Ignore_ssl         bool   `json:"ignore_ssl" yaml:"ignore_ssl"`
	dry_run            bool
	verbose            bool
}
...
func CreateK8KubeConfig(default_context string, config_path string) *K8 {
	return &K8{
...
// ignore_ssl if true, ignore the ssl connection
func CreateK8Token(host string, auth string, ignore_ssl bool) *K8 {
	return &K8{
		Host:               host,
		Authorization:      auth,
